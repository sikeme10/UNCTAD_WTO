...
reg1 <- feglm(total_NTM_BRA_TBT ~ total_NTM_CHL_TBT + total_NTM_BOL_TBT + total_NTM_COL_TBT +
...
merged_UNCTAD_H5 <- merged_UNCTAD_NTM %>%
  ...
  summarize(total_NTM_BRA_TBT = sum(total_NTM_BRA_TBT , na.rm = TRUE),
            ...
            #total_NTM_ARG_TBT = sum(total_NTM_ARG_TBT , na.rm = TRUE),
            total_NTM_PER_TBT = sum(total_NTM_PER_TBT , na.rm = TRUE))
names(merged_UNCTAD_H5)
str(merged_UNCTAD_H5)


reg_H5 <- feglm(total_NTM_BRA_TBT ~ 0 + H2*total_NTM_CHL_TBT + H2*total_NTM_BOL_TBT + H2*total_NTM_COL_TBT +
                  H2*total_NTM_ECU_TBT + H2*total_NTM_PER_TBT,
                data = merged_UNCTAD_H5, family = poisson(link = "log"))
...
# rename variable
BRA_UNCTAD_TBT_count <- all_sub_Merge_UNCTAD %>% rename(PartnerISO3 = Partner,
                                                        ntm_count_TBT = predict_BRA)
names(BRA_UNCTAD_TBT_count)
...
# select the NTM data:'
BRA_UNCTAD_TBT_count <- BRA_UNCTAD_TBT_count %>% group_by(Year, H5) %>%
  select(Year, H5, ntm_count_TBT)
#join the two data sets
Weights_BRA_trade <- full_join(Weights_BRA_trade,BRA_UNCTAD_TBT_count, relationship = "many-to-many")
# recalculate NTM counts with the weight:
Weights_BRA_trade$NTM_TBT_weigthed <- Weights_BRA_trade$ntm_count_TBT * Weights_BRA_trade$weights_2012
Weights_BRA_trade <- Weights_BRA_trade %>% group_by(ReporterISO3, Year, H5) %>% 
  summarize( ntm_count_TBT_weight =  sum(NTM_TBT_weigthed))
...
write.csv(Merge_UNCTAD_IV, "BRA/IVs/IV_TBT_UNCTAD.csv")
