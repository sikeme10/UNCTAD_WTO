...
merged_UNCTAD_NTM_H5 <- merged_UNCTAD_NTM %>% group_by(Year, Partner, HSCode) %>%
  summarise(total_NTM_BRA_SPS_H5 = sum (total_NTM_BRA_SPS),
            total_NTM_CHL_SPS_H5 = sum (total_NTM_CHL_SPS),
            total_NTM_BOL_SPS_H5 = sum (total_NTM_BOL_SPS),
            total_NTM_COL_SPS_H5 = sum (total_NTM_COL_SPS),
            total_NTM_ECU_SPS_H5 = sum (total_NTM_ECU_SPS),
            total_NTM_PER_SPS_H5 = sum (total_NTM_PER_SPS))


merged_UNCTAD_NTM_H5 <-merged_UNCTAD_NTM_H5 %>% rename(PartnerISO3 =Partner)
length(unique(merged_UNCTAD_NTM_H5$PartnerISO3))


# get years from 2012 to 2016
merged_UNCTAD_NTM_H5 <- subset(merged_UNCTAD_NTM_H5, Year< 2017)

# need to get rid of WLD as partner:
table(merged_UNCTAD_NTM_H5$PartnerISO3)
...
SPS_UNCTAD <- WLD_partner(BRA_trade_Merge, merged_UNCTAD_NTM_H5)
names(SPS_UNCTAD)

#calculate counts of NTM by partners and HS code 
SPS_UNCTAD <- SPS_UNCTAD %>%  group_by(Year, PartnerISO3, H5) %>%
  summarise(total_NTM_BRA_SPS_H5 = sum(total_NTM_BRA_SPS_H5, na.rm = TRUE),
            total_NTM_CHL_SPS_H5 = sum(total_NTM_CHL_SPS_H5, na.rm = TRUE),
            total_NTM_BOL_SPS_H5 = sum(total_NTM_BOL_SPS_H5, na.rm = TRUE),
            total_NTM_COL_SPS_H5 = sum(total_NTM_COL_SPS_H5, na.rm = TRUE),
            total_NTM_ECU_SPS_H5 = sum(total_NTM_ECU_SPS_H5, na.rm = TRUE),
            total_NTM_PER_SPS_H5 = sum(total_NTM_PER_SPS_H5, na.rm = TRUE))
...
names(merged_UNCTAD_NTM_H5)
class(Weighting_trade$H5)

weighted_NTM  <- left_join(merged_UNCTAD_NTM_H5, Weighting_trade)
...
# Get HS levels variables
SPS_UNCTAD$H1<- substring(SPS_UNCTAD$H5, first = 1, last = 1)
SPS_UNCTAD$H2<- substring(SPS_UNCTAD$H5, first = 1, last = 2)
SPS_UNCTAD$H3<- substring(SPS_UNCTAD$H5, first = 1, last = 3)
SPS_UNCTAD$H4<- substring(SPS_UNCTAD$H5, first = 1, last = 4)
...
for (H1_val in unique_H1) {
  ...
  reg_H5 <- feols(total_NTM_BRA_SPS_H5 ~ 0+  total_NTM_CHL_SPS_H5 + total_NTM_BOL_SPS_H5 + total_NTM_COL_SPS_H5 +
                    total_NTM_ECU_SPS_H5 + total_NTM_PER_SPS_H5,
...
for (h1_val in unique_H1) {
  ...
  newdata <- data.frame(Year = sub_Merge_UNCTAD$Year,
                        ...
                        total_NTM_BRA_SPS_H5 = sub_Merge_UNCTAD$total_NTM_BRA_SPS_H5,
                        total_NTM_CHL_SPS_H5 = sub_Merge_UNCTAD$total_NTM_CHL_SPS_H5,
                        total_NTM_BOL_SPS_H5 = sub_Merge_UNCTAD$total_NTM_BOL_SPS_H5,
                        total_NTM_COL_SPS_H5 = sub_Merge_UNCTAD$total_NTM_COL_SPS_H5,
                        total_NTM_ECU_SPS_H5 = sub_Merge_UNCTAD$total_NTM_ECU_SPS_H5,
                        total_NTM_PER_SPS_H5 = sub_Merge_UNCTAD$total_NTM_PER_SPS_H5)
...
BRA_UNCTAD_SPS_count <- select(BRA_UNCTAD_SPS_count, Year , PartnerISO3, H5, ntm_count_SPS )
...
merged_UNCTAD_NTM_H5 <- merged_UNCTAD_NTM %>% group_by(Year, Partner, H5) %>%
  summarise(total_NTM_BRA_TBT_H5 = sum (total_NTM_BRA_TBT),
            total_NTM_CHL_TBT_H5 = sum (total_NTM_CHL_TBT),
            total_NTM_BOL_TBT_H5 = sum (total_NTM_BOL_TBT),
            total_NTM_COL_TBT_H5 = sum (total_NTM_COL_TBT),
            total_NTM_ECU_TBT_H5 = sum (total_NTM_ECU_TBT),
            total_NTM_PER_TBT_H5 = sum (total_NTM_PER_TBT))
...
# Call the function with the provided dataframes
TBT_UNCTAD <- WLD_partner(BRA_trade_Merge, merged_UNCTAD_NTM_H5)
names(TBT_UNCTAD)

#calculate counts of NTM by partners and HS code 
TBT_UNCTAD <- TBT_UNCTAD %>%  group_by(Year, PartnerISO3, H5) %>%
  summarise(total_NTM_BRA_TBT_H5 = sum(total_NTM_BRA_TBT_H5, na.rm = TRUE),
            total_NTM_CHL_TBT_H5 = sum(total_NTM_CHL_TBT_H5, na.rm = TRUE),
            total_NTM_BOL_TBT_H5 = sum(total_NTM_BOL_TBT_H5, na.rm = TRUE),
            total_NTM_COL_TBT_H5 = sum(total_NTM_COL_TBT_H5, na.rm = TRUE),
            total_NTM_ECU_TBT_H5 = sum(total_NTM_ECU_TBT_H5, na.rm = TRUE),
            total_NTM_PER_TBT_H5 = sum(total_NTM_PER_TBT_H5, na.rm = TRUE))
...
# Get HS levels variables
TBT_UNCTAD$H1<- substring(TBT_UNCTAD$H5, first = 1, last = 1)
TBT_UNCTAD$H2<- substring(TBT_UNCTAD$H5, first = 1, last = 2)
TBT_UNCTAD$H3<- substring(TBT_UNCTAD$H5, first = 1, last = 3)
TBT_UNCTAD$H4<- substring(TBT_UNCTAD$H5, first = 1, last = 4)
...
for (h2_val in unique_H1) {
  ...
  reg_H5 <- feols(total_NTM_BRA_TBT_H5 ~ 0+  total_NTM_CHL_TBT_H5 + total_NTM_BOL_TBT_H5 + total_NTM_COL_TBT_H5 +
                    total_NTM_ECU_TBT_H5 + total_NTM_PER_TBT_H5,
                  data = sub_Merge_UNCTAD)
...
for (h1_val in unique_H1) {
  ...
  newdata <- data.frame(Year = sub_Merge_UNCTAD$Year,
                        PartnerISO3 = sub_Merge_UNCTAD$PartnerISO3,
                        H5 = sub_Merge_UNCTAD$H5,
                        total_NTM_BRA_TBT_H5 = sub_Merge_UNCTAD$total_NTM_BRA_TBT_H5,
                        total_NTM_CHL_TBT_H5 = sub_Merge_UNCTAD$total_NTM_CHL_TBT_H5,
                        total_NTM_BOL_TBT_H5 = sub_Merge_UNCTAD$total_NTM_BOL_TBT_H5,
                        total_NTM_COL_TBT_H5 = sub_Merge_UNCTAD$total_NTM_COL_TBT_H5,
                        total_NTM_ECU_TBT_H5 = sub_Merge_UNCTAD$total_NTM_ECU_TBT_H5,
                        total_NTM_PER_TBT_H5 = sub_Merge_UNCTAD$total_NTM_PER_TBT_H5)
...
BRA_UNCTAD_TBT_count <- select(BRA_UNCTAD_TBT_count, Year , PartnerISO3,H5, ntm_count_TBT )
