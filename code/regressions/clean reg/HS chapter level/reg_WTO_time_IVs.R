...
table(Merge$H2)
Merge$H2_num <- as.numeric(Merge$H2)
table(Merge$H2_num)
...
Merge <- Merge %>% arrange(HS_section , Year, PartnerISO3) 
unique_HS <- unique(Merge$HS_section)
...
for (HS_val in unique_HS) {
  ...
  sub_merge <- subset(Merge, HS_section == HS_val)
  
  # Run the regression
  reg <- feols(log(price) ~
                 log(dist) + contig + comlang_off + rta + landlocked_rep + landlocked_par +
                 ntm_count_SPS + ntm_count_TBT + log(1 + tariff_BRA) + Reporter_GDP + 
...
names(summary_table)
write.table(summary_table_2, file = "/data/sikeme/TRADE/WTO/result/regression_HS_section/regression_results_HS_section_WTO_IV.csv", sep = ",", row.names = FALSE)
...
combined_df_AVE_WTO_SPS <- rownames_to_column(combined_df_AVE_WTO_SPS, var = "HS_section")
write.csv(combined_df_AVE_WTO_SPS, file = "/data/sikeme/TRADE/WTO/result/regression_HS_section/AVE_SPS_WTO_IV.csv",  row.names = TRUE)
...
combined_df_AVE_WTO_TBT <- rownames_to_column(combined_df_AVE_WTO_TBT, var = "HS_section")
write.csv(combined_df_AVE_WTO_TBT, file = "/data/sikeme/TRADE/WTO/result/regression_HS_section/AVE_TBT_WTO_IV.csv",  row.names = TRUE)
...
for (year in unique(Merge$Year)) {
  ...
  Count_HS_year_WTO <- Count_HS_year_WTO$ntm_count_SPS_H1
  df_list_AVE_SPS <- list()
  # Loop through elements for the current year
  for (i in seq_along(Count_HS_year_WTO)) {
    val <- Count_HS_year_WTO[i]
    
    # Check if "ntm_count_SPS" exists in the coefficients of the regression summary
    if ("ntm_count_SPS" %in% names(regression_summaries[[i]]$coefficients)) {
      # Calculate AVE_SPS
      AVE_SPS <- deltaMethod(regression_summaries[[i]], "((exp(ntm_count_SPS) * val - 1)) * 100")
      df_AVE_SPS <- data.frame(AVE_SPS)
    } else {
      # Assign default values if "ntm_count_SPS" is not present
      df_AVE_SPS <- data.frame(Estimate = 0, SE = NA, `2.5 %` = NA, `97.5 %` = NA)
    }
    
    # Set row names and store in the list
