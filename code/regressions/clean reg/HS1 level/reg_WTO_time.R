...
for (H1_val in unique_H1) {
  ...
  sub_merge <- subset(Merge, H1 == H1_val)
...
summary_table_2  <- rbind(summary_table[1:15, ], summary_table[5193:5195, ])
names(summary_table)
write.table(summary_table_2, file = "/data/sikeme/TRADE/WTO/result/regression_results_H1_WTO.csv", sep = ",", row.names = FALSE)
...
write.table(combined_df_AVE_WTO_SPS, file = "/data/sikeme/TRADE/WTO/result/AVE_SPS_H1_WTO.csv", sep = ",", row.names = TRUE)
...
write.table(combined_df_AVE_WTO_TBT, file = "/data/sikeme/TRADE/WTO/result/AVE_TBT_H1_WTO.csv", sep = ",", row.names = TRUE)
